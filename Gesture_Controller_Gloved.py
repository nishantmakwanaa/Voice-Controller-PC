...
def draw_box(frame, points, color=(0, 255, 127)):
    if points:
        frame = cv2.line(
            frame, points[0], points[1], color, thickness=2, lineType=8
        )  # top
        frame = cv2.line(
            frame, points[1], points[2], color, thickness=2, lineType=8
        )  # right
        frame = cv2.line(
            frame, points[2], points[3], color, thickness=2, lineType=8
        )  # bottom
        frame = cv2.line(
            frame, points[3], points[0], color, thickness=2, lineType=8
        )  # left
...
class ROI:
    ...
    def findROI(self, frame, marker):
        ...
        bot_ry = int(
            cy + self.roi_alpha2 * slope_12 * l * np.sqrt(1 / (1 + slope_12**2))
        )

        bot_lx = int(cx - self.roi_alpha1 * l * np.sqrt(1 / (1 + slope_12**2)))
        bot_ly = int(
            cy - self.roi_alpha1 * slope_12 * l * np.sqrt(1 / (1 + slope_12**2))
        )

        top_lx = int(bot_lx + sign * self.roi_beta * l * np.sqrt(1 / (1 + slope_14**2)))
        top_ly = int(
            bot_ly
            + sign * self.roi_beta * slope_14 * l * np.sqrt(1 / (1 + slope_14**2))
        )
    ...
    def find_glove_hsv(self, frame, marker):
        ...
        bot_rx = int(
            self.marker_top[0] + self.hsv_alpha * l * np.sqrt(1 / (1 + slope_12**2))
        )
        bot_ry = int(
            self.marker_top[1]
            - self.hsv_lift_up * l
        ...
        top_ly = int(
            bot_ly
            + sign * self.hsv_beta * slope_14 * l * np.sqrt(1 / (1 + slope_14**2))
        )
    ...
    def find_gesture(self, frame):
        ...
        if self.fingers == 1:
            ...
            if self.arearatio < 15:
                ...
                cv2.putText(
                    frame, "2 fingers", (0, 50), font, 2, (0, 0, 255), 3, cv2.LINE_AA
                )
                self.gesture = 2
            else:
                cv2.putText(
                    frame, "1 finger", (0, 50), font, 2, (0, 0, 255), 3, cv2.LINE_AA
                )
                self.gesture = 1
...
class Tracker:
    ...
    def tracker_to_corner(self, final_bbox):
        ...
        final_bbox[0][1] = [
            self.tracker_bbox[0] + self.tracker_bbox[2],
            self.tracker_bbox[1],
        ]
        final_bbox[0][2] = [
            self.tracker_bbox[0] + self.tracker_bbox[2],
            self.tracker_bbox[1] + self.tracker_bbox[3],
        ]
        final_bbox[0][3] = [
            self.tracker_bbox[0],
            self.tracker_bbox[1] + self.tracker_bbox[3],
    ...
    def CSRT_tracker(self, frame):
        ...
        if ok:
            ...
            p2 = (
                int(self.tracker_bbox[0] + self.tracker_bbox[2]),
...
class GestureController:
    ...
    def start(self):
        while True:
            ...
            GestureController.f_start_time = time.time()
            while (
                GestureController.f_now_time - GestureController.f_start_time
                <= 1.0 / fps
            ):
                GestureController.f_now_time = time.time()
            ...
            if GestureController.aru_marker.is_detected():
                GestureController.csrt_track.corners_to_tracker(
                ...
                GestureController.csrt_track.CSRT_tracker(frame)
                GestureController.aru_marker.corners = (
                    GestureController.csrt_track.tracker_to_corner(
                        GestureController.aru_marker.corners
                    )
                )
            ...
            if GestureController.aru_marker.is_detected():
                ...
                GestureController.hand_roi.find_glove_hsv(
                    frame, GestureController.aru_marker
                )
